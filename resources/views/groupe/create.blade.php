...
<div class="card">
    <header class="card-header">
        <p class="card-header-title">Création d'un groupe</p>
    </header>
...
<form action="{{ route('groupes.store') }}" method="POST">
    ...
    <div class="field">
        ...
        <div class="control">
          <textarea class="textarea @error('description') is-danger @enderror" type="text" name="description" value="{{ old('description') }}" placeholder="la description  "></textarea>
        </div>
        @error('description')
            <p class="help is-danger">{{ $message }}</p>
        @enderror
